...
void checkPrimalFeasMatrix(const State& state, KktConditionDetails& details) {
  ...
  for (int i = 0; i < state.numRow; i++) {
    if (state.flagRow[i]) {
      ...
      if (((rowV - state.rowLower[i]) < 0) &&
        ...
        if (dev_print == 1)
          Rcpp::Rcout << "Row " << i << " infeasible: Row value=" << rowV
                    << "  L=" << state.rowLower[i]
  ...
  if (details.violated == 0) {
    if (dev_print == 1) Rcpp::Rcout << "Primal feasible.\n";
  } else {
    if (dev_print == 1) Rcpp::Rcout << "KKT check error: Primal infeasible.\n";
  }
}
...
void checkDualFeasibility(const State& state, KktConditionDetails& details) {
    ...
    if (state.flagCol[i]) {
      ...
      if (state.colLower[i] <= -kHighsInf && state.colUpper[i] >= kHighsInf) {
        if (fabs(state.colDual[i]) > tol) {
          if (dev_print == 1)
            Rcpp::Rcout << "Dual feasibility fail: l=-inf, x[" << i
      ...
      // j in L: x=l and l<u
        ...
        if (state.colDual[i] < 0 && fabs(state.colDual[i]) > tol) {
          if (dev_print == 1)
            Rcpp::Rcout << "Dual feasibility fail: l[" << i
      ...
      // j in U: x=u and l<u
        ...
        if (state.colDual[i] > tol) {
          if (dev_print == 1)
            Rcpp::Rcout << "Dual feasibility fail: x[" << i
    ...
    if (state.flagRow[i]) {
      ...
      if (fabs(state.rowLower[i] - rowV) < tol && rowV < state.rowUpper[i]) {
        if (state.rowDual[i] < -tol) {
          if (dev_print == 1)
            Rcpp::Rcout << "Dual feasibility fail for row " << i
  ...
  if (details.violated == 0) {
    if (dev_print == 1) Rcpp::Rcout << "Dual feasible.\n";
  } else {
    if (dev_print == 1)
      Rcpp::Rcout << "KKT check error: Dual feasibility fail.\n";
  }
}
...
void checkComplementarySlackness(const State& state,
    ...
    if (state.flagCol[i]) {
      ...
      if (state.colLower[i] > -kHighsInf &&
        ...
        if (fabs(state.colDual[i]) > tol &&
          ...
          if (dev_print)
            // clang-format off
            Rcpp::Rcout << "Comp. slackness fail: "
      ...
      if (state.colUpper[i] < kHighsInf &&
        ...
        if (fabs(state.colDual[i]) > tol &&
            fabs(state.colValue[i] - state.colLower[i]) > tol) {
          if (dev_print == 1)
            Rcpp::Rcout << "Comp. slackness fail: x[" << i
  ...
  if (details.violated == 0) {
    if (dev_print == 1) Rcpp::Rcout << "Complementary Slackness.\n";
  } else {
    if (dev_print == 1) Rcpp::Rcout << "KKT check error: Comp slackness fail.\n";
  }
}
...
void checkStationarityOfLagrangian(const State& state,
  ...
  if (details.violated == 0) {
    if (dev_print == 1) Rcpp::Rcout << "Stationarity of Lagrangian.\n";
  } else {
    if (dev_print == 1)
      Rcpp::Rcout << "KKT check error: Lagrangian is not stationary.\n";
  }
}
...
void checkBasicFeasibleSolution(const State& state,
  ...
  for (int j = 0; j < state.numCol; j++) {
    if (state.flagCol[j]) {
      ...
      if (state.col_status[j] == HighsBasisStatus::kBasic &&
        ...
        if (dev_print == 1)
          Rcpp::Rcout << "Col " << j << " is basic but has nonzero dual "
  ...
  for (int i = 0; i < state.numRow; i++) {
    if (state.flagRow[i]) {
      ...
      if (state.row_status[i] == HighsBasisStatus::kBasic &&
        ...
        if (dev_print == 1)
          Rcpp::Rcout << "Row " << i << " is basic but has nonzero dual: "
  ...
  if (details.violated == 0) {
    if (dev_print == 1) Rcpp::Rcout << "BFS." << std::endl;
  } else {
    if (dev_print == 1)
      Rcpp::Rcout << "BFS X Violated: " << details.violated << std::endl;
  }
  ...
  if (!holds) {
    details.violated = -1;
    Rcpp::Rcout << "BFS X Violated WRONG basis count: "
...
bool checkKkt(const State& state, KktInfo& info) {
  if (state.numCol == 0) {
    Rcpp::Rcout << "KKT warning: empty problem" << std::endl;
    return true;
  }
