...
HighsLpRelaxation::Status HighsLpRelaxation::run(bool resolve_on_error) {
  ...
  if (dev_report && !mipsolver.submip) {
    if (valid_basis) {
      Rprintf("Solving LP (%7d, %7d) with    a valid basis\n",
             int(lpsolver.getNumCol()), int(lpsolver.getNumRow()));
    } else {
      Rprintf("Solving LP (%7d, %7d) without a valid basis\n",
             int(lpsolver.getNumCol()), int(lpsolver.getNumRow()));
    }
  }
  ...
  switch (model_status) {
    ...
    case HighsModelStatus::kInfeasible: {
      ...
      // Rprintf("error: unreliable infeasibilities, modelstatus = %"
    ...
    case HighsModelStatus::kIterationLimit: {
      ...
      // Rprintf("error: lpsolver reached iteration limit\n");
      return Status::kError;
    }
    case HighsModelStatus::kTimeLimit:
      return Status::kError;
    default:
      // Rprintf("error: lpsolver stopped with unexpected status %"
      // HIGHSINT_FORMAT "\n",
    ...
    switch (status) {
      ...
      case Status::kOptimal: {
        ...
        if (domain && !domain->getChangedCols().empty()) {
          // Rprintf("resolving due to fixings of substituted columns\n");
          solveagain = true;
          continue;
        }
