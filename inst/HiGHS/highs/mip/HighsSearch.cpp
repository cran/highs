...
HighsInt HighsSearch::selectBranchingCandidate(int64_t maxSbIters,
  ...
  while (true) {
    ...
    if (!downscorereliable[candidate] &&
      ...
      // if (!mipsolver.submip)
      //   Rprintf("down eval col=%d fracval=%g\n", col, fracval);
      if (strongBranch(false)) return -1;
    } else {
      // if (!mipsolver.submip)
      //  Rprintf("up eval col=%d fracval=%g\n", col, fracval);
      // evaluate up branch
...
HighsSearch::NodeResult HighsSearch::branch() {
  ...
  while (nodestack.back().opensubtrees == 2 &&
    ...
    // if (!mipsolver.submip)
    //  Rprintf("selecting branching cand with minrel=%d\n",
    ...
    // if (!mipsolver.submip)
    //   Rprintf("branching cand returned as %d\n", branchcand);
    NodeData& currnode = nodestack.back();
  ...
  while (true) {
    ...
    if (!nodeToQueue) {
      for (HighsInt i = nodestack.size() - 2; i >= 0; --i) {
        ...
        // if (!mipsolver.submip)
        //   Rprintf("nodeScore: %g, ancestorScore: %g\n", nodeScore,
        //   ancestorScore);
    ...
    if (nodeToQueue) {
      // if (!mipsolver.submip) Rprintf("node goes to queue\n");
      std::vector<HighsInt> branchPositions;
      auto domchgStack = localdom.getReducedDomainChangeStack(branchPositions);
      double tmpTreeWeight = nodequeue.emplaceNode(
