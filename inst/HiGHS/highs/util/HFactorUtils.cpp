  ...
  if (l_u_or_both & 2) {
    ...
    if (full) {
      ...
      for (size_t iRow = 0; iRow < ur_start.size(); iRow++) {
        ...
        Rprintf("UR    Row %2d: ", (int)iRow);
        for (HighsInt iEl = start; iEl < end; iEl++)
          Rprintf("%11d ", (int)ur_index[iEl]);
        Rprintf("\n              ");
        for (HighsInt iEl = start; iEl < end; iEl++)
          Rprintf("%11.4g ", ur_value[iEl]);
        Rprintf("\n");
      }
...
void HFactor::reportIntVector(const std::string name,
  ...
  Rprintf("%-12s: siz %4d; cap %4d: ", name.c_str(), (int)num_en,
         (int)entry.capacity());
  for (HighsInt iEn = 0; iEn < num_en; iEn++) {
    if (iEn > 0 && iEn % 10 == 0)
      Rprintf("\n                                  ");
    Rprintf("%11d ", (int)entry[iEn]);
  }
  Rprintf("\n");
}
...
void HFactor::reportAsm() {
  for (HighsInt count = 1; count <= num_row; count++) {
    ...
    for (HighsInt j = col_link_first[count]; j != -1; j = col_link_next[j]) {
      ...
      Rprintf("Col %4d: count = %2d; min_pivot = %10.4g; [%4d, %4d)\n", (int)j,
      ...
      for (HighsInt k = start; k < end; k++) {
        ...
        Rprintf("   Row %4d; Count = %2d; Merit = %11.4g; Value = %11.4g: %s\n",
